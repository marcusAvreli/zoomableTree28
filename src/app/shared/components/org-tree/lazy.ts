...
export class LazyOrgChart<Datum extends {}> extends OrgChart<Datum> {
    ...
    addNodes(nodes: Datum[]) {
        for (const obj of nodes) {
            const attrs = (this as any)['getChartState']();
        ...
        this.callUpdateNodesState();
        return this;
    };

     onButtonClick(event :any, d:any) {
        const attrs = this.getLazyLoadingAttrs();
	//	console.log("onButtonClick",attrs, " d:",d,attrs.hasChildren(d.data));
        if ((!d.data.children || !d.children) && !(d as any)._children && attrs.hasChildren(d.data)) {
			//console.log("onButtonClick"," passedCondition",attrs, " d:",d,attrs.hasChildren(d.data));
            attrs.loadChildren(d.data)
                .then(nodes => {
					//console.log("onButtonClick"," addReceivedNodes:",nodes);
                    this.addNodes(nodes.map(node => {
                        (node as any)._expanded = true;
                        return node;
                    }));
                    super.onButtonClick(event, d);
                });
        }
        else {
            super.onButtonClick(event, d);
     ...
     update(params: { x0: number; y0: number; width: number; height: number; } & Partial<any>) {
		// console.log("params: ",params);
        super.update(params);
        this.getLazyLoadingAttrs().afterUpdate?.();
        const container = select((this as any)['getChartState']().container).node() as Element;
        if (container)
container.querySelectorAll(".node-button-g").forEach(node => {
  if (node.querySelector(".node-button-div")?.childElementCount) {
	...
	private callUpdateNodesState() {
  return (this as any)['updateNodesState']();
}
}