...
console.log("onSelectedNodeChanged:",nodes,this.nodeMap);
    }),
  ...
  triggerSearch() {
    if (!this.searchField || !this.searchTerm.trim()) return;
    this.handleSearch({ term: this.searchTerm, field: this.searchField });
  }
  /* ---------------- Search Entry ---------------- */
...
private emitState() {
  this.chartStateSubject.next({
    root: this.nodeMap.get('root') ?? null,
    highlightedIds: new Set(this.highlightedNodeIds),
    searchScope: new Set(this.searchScope),
    loadChildren$: this.loadChildren$,
	 nodeMap: new Map(this.nodeMap), // pass full nodeMap
	   ancestor: this.currentAncestor,   // 🔥 HERE
	   rootKey: 'root'
  });
}
